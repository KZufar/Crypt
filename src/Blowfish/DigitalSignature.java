...
public class DigitalSignature {
    ...
    public void Sign(String input, byte[] hash) {
        ...
        // 17 схема цифровой подписи
        s = ((k.subtract(po.multiply(h))).multiply((h.multiply(x)).modPow(new BigInteger("-1"), q))).mod(q);
        ...
        //Проверка подписи
        d = r;
        e = ((g.modPow(po.multiply(h), p)).multiply(y.modPow(s.multiply(h), p))).mod(p);

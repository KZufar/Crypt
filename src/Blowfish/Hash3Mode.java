...
public class Hash3Mode {
    public static byte[] HashFunction(byte[] text, blowfish cipher, String h0) {
        byte[] sB = h0.getBytes();
        ...
        for (int i=0; i<blockedText.size(); i++){
            // используется 3 схема безопасного хэширования
            byte[] mi = blockedText.get(i);
            byte[] miXorH = xor(blockedText.get(i), h);
            h = xor(cipher.decipher(mi), miXorH);
        }
        return h;
    }
